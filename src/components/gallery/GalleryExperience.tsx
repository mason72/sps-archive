...
export type GallerySource =
  ...
  | { kind: "preview"; eventId: string };

/**
 * What this surface is ALLOWED to do. Preview is the same gallery, read-only:
 * a photographer checking their work must not write guest favorites, inflate
 * the share's view count, or be asked for a password they set for clients.
 * Everything a guest can SEE, preview shows.
 */
function capabilitiesFor(source: GallerySource) {
  const isShare = source.kind === "share";
  return {
    /** Favorites write to a guest identity that a preview doesn't have. */
    favorites: isShare,
    /** View counts and activity are a record of GUESTS, not of the owner. */
    tracking: isShare,
    /** The password gate protects a share; the owner is already authed. */
...
/**
 * The gallery, once — rendered for guests at /gallery/[slug] and for the
 * photographer at /gallery/preview/[eventId].
 *
 * It used to be two files: this one, and a 609-line copy that had drifted
 * into filename-only search with no selfie search at all, so a preview was
 * quietly NOT what a guest sees (Mason, 2026-08-10: "previewing a gallery
 * should have 100% of the fidelity of viewing a real gallery"). One
 * component, one API base, and the only differences are the four
 * capabilities above — every future feature lands on both surfaces by
 * construction.
 */
export function GalleryExperience({ source }: { source: GallerySource }) {
  // Every call this component makes hangs off ONE base. That is the whole
  // reason the two surfaces can share an implementation.
  const apiBase =
    source.kind === "share"
      ? `/api/gallery/${source.slug}`
      : `/api/gallery/preview/${source.eventId}`;
  const can = capabilitiesFor(source);
  // PasswordGate posts to the share-verify route by slug — share-only, and
  ...
  const syncFavoriteToServer = (
    ...
    // Belt and braces: the UI already hides favorites in preview, but a write
    // path is exactly the thing that must fail closed rather than 404 loudly.
    if (!can.favorites) return;
  ...
  const pollZipJob = (jobId: string, token: string | null, imageTotal?: number) => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${apiBase}/download/status?${params.toString()}`
        );
        const data = await res.json();
  ...
  const startBulkDownload = async (
    ...
    try {
      const res = await fetch(`${apiBase}/download/prepare`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.mode === "direct") {
        ...
        window.location.href = `${apiBase}/download${qs ? `?${qs}` : ""}`;
        return;
      }
      // Background build
      if (data.status === "ready" && data.url) {
  ...
  const deliverImageDownload = async (
    ...
    if (!url) {
      try {
        const res = await fetch(`${apiBase}/image-download`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            imageId: image.id,
    ...
    // Track download (fire-and-forget via sendBeacon). Never from a preview —
    // the owner checking their own gallery is not a guest download.
    try {
      if (can.tracking) navigator.sendBeacon(
        `${apiBase}/track`,
        new Blob(
  ...
  // Password gate. Only a share can be locked — the preview API answers to an
  // authenticated owner and never returns requiresAuth, but the capability
  // makes that a stated rule rather than a fact about another file.
  if (can.passwordGate && requiresAuth && authMeta) {
    return (
      <PasswordGate
        slug={slug}
  ...
  // The two guest permissions, resolved ONCE: what the photographer allowed,
  // narrowed by what this surface can do at all. Every consumer below reads
  // these rather than `gallery.*`, so preview can't sprout a favorite button
        ...
        {searchQuery.trim() ? (
          filteredImages.length > 0 ? (
            <>
              ...
              onFavoriteMany={allowFavorites ? handleFavoriteMany : undefined}
              onImageClick={(id) => setSelectedImageId(id)}
              onDownloadClick={allowDownload ? handleIndividualDownload : undefined}
              selectedIds={selectedIds}
              onToggleSelect={toggleSelect}
              onOpenStack={setOpenStack}
              onDownloadStack={allowDownload ? handleStackDownload : undefined}
