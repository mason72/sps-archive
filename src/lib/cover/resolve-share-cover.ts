...
/**
 * The ids a share is allowed to show, in curation order, or null for "no
 * restriction". A selection share with an empty/absent id list is treated as
 * unrestricted — that's the shape a full share has, and it is what every
 * other gallery path already assumes.
 */
export function selectionScope(share: ShareCoverRef): string[] | null {
  return share.share_type === "selection" &&
...
export async function resolveShareCoverUrl(
  ...
  const scopeIds = selectionScope(share);
