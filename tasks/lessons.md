...
**Two sessions fixed the same bug in the same hour.** My push was rejected: a concurrent session had shipped lesson 152's `nameHasSessionWord` (a vocabulary veto: session, breakout, reception, team, group…), measured at 38 cards. I rebased and measured only what mine ADDS, building main and my tree back to back: 4 cards (Guardant NSM GS, eBay HR, Atlassian Braindates, FM Logo), all inside the 26 Mason approved. Three small "removed" names (Robin Barrett, Rebeca Jefferson, Stacy Burrier) were dated BIO-TECHNE files uploaded during the builds; a dated file can never match this rule, so that was drift by construction. The rules are complements, not duplicates: vocabulary catches dated and non-event-word labels ("Team Shots", "Name Unknown"), file shape catches sessions with no session word. The first control build failed silently behind a `tail -1`, which broke the `&&` chain; read a build's whole output before chaining anything on it.

**Rules.**
- **When a push is rejected, read what landed before rebasing.** Here it was the same fix by another route; the right question became "what does mine add on top", measured, not "resolve the conflict".
...
**Addendum (same day): the deploy did not register the new function.** Vercel went Ready, but Inngest never learned `sps-pull-watchdog` existed: a manual `sps/pull-watchdog.run` event produced no run in 10 minutes. `curl -X PUT https://app.pixeltrunk.com/api/inngest` answered `{"modified":true}`, which proves the function list had been stale, and the next trigger ran clean (`checked 2, restarted 0, alerted 0`). **After any deploy that ADDS an Inngest function, PUT the serve endpoint and trigger it once.** A new function that is not registered fails in total silence: no error, no run. Existing functions hide this because they were registered long ago. Also measured: events wait ~3–7 min in `Scheduled` before starting on a busy afternoon, which is normal here, well inside the 30-minute stall window.

**Automated (same day):** `.github/workflows/inngest-sync.yml` runs on GitHub's `deployment_status` event (Vercel reports each deploy there). On a successful `Production` deploy it PUTs the serve endpoint, retries 5×, and FAILS unless the response says "Successfully registered". Negative-tested against a wrong path (exit 1) before shipping. Note: `modified:true` also appears when only the registered URL changes (the integration registers per-deployment URLs, this job the production domain), so it does not by itself mean a function changed. The 401 on an unsigned `GET /api/inngest` is the Inngest SDK refusing an unsigned request, not Vercel protection: the body is JSON from the function in pdx1.

## 161 — Six workers, one read-add-write, and a counter that ran short (2026-09-18)

**What happened.** Two completed SPS imports reported fewer photos than landed. "Everpure GSO Onboarding Headshots": `images_done` 1,080 against 1,090 rows, all with distinct `sps_image_id`, 0 failed, 0 skipped, never restarted. Autodesk University 2026: 6,107 against 6,112.

**The screen was not the problem.** The import screen's headline has read `landed` (a row count) since `bf6667c` (2026-08-11), so it showed 1,090 / 1,090. The counter fed the "Stopped. The N photos" line, the watchdog's progress signal and the job summary. Check which surface a number actually reaches before fixing "the screen".

**Cause, with evidence.** `applySliceResult` read the job row, added in JS and wrote the sum. Its comment called that safe because "a job runs with one worker". That is true across Inngest runs, and false inside a slice: `importSlice` runs 6 downloads and flushes every 5 photos, so two flushes overlap, both read the same row, and the later write erases the earlier one. The other suspect was a failed flush, which was claimed and then dropped. It was ruled out: today's production error log was complete (26 entries against a limit of 100) and had no "flush failed" line. Supporting evidence: every loss was a multiple of 5, and Everpure landed 5 photos within 300 ms three times. `scripts/triage/sps-pull-progress-race.ts` races both shapes against a real row. The old fold recorded **15 of 300**.

**Fix.**
- **Migration 085, `sps_pull_add_progress`.** One `UPDATE … SET images_done = images_done + $n`. Postgres serializes concurrent updates of one row on the row lock, so there is no window. The race probe: **300 of 300**.
- **`createProgressFlusher()`** claims each delta before the await and hands it back on a failed write by SUBTRACTING, not by restoring a snapshot. A restore would re-send another flush's claim.
- **The import screen** names a total that was counted too low ("2 more than the 6,110 counted when the import started, usually photos added in SimplePhotoShare along the way") instead of printing 6,112 / 6,110. For Autodesk that cause is certain: it had one job and no deselected photo landed. The copy says "usually" because `landed` counts every SPS row in the event, and rows left by an earlier import would also trip it. Also noted in review: a write that commits but loses its response is re-sent, so the counter can now run OVER by one flush. That is the better failure of the two.
...
## 169 — The pre-push live check printed "null" mid-ingest and called it a result (2026-09-24)
**What happened.** `scripts/triage/live-activity.ts`, the documented pre-push check for live events, printed `images uploaded in last 6h : null` while the Pixieset ingest was writing images minutes apart. It ran an exact count over ~600k `images` rows with no usable index on `created_at` alone; under ingest load that hit the 15 s service-role statement timeout, and the script destructured only `count`, so the error vanished and `null` printed where a number belonged. "Events dated today: 0" had the same flaw. The check also could not have answered its real question even when it worked: thousands of migration uploads an hour would have read as a live event.

**Fix.** Every query goes through `must()`, which throws on `error`; any failure exits **1** with "the answer is unknown, not quiet". Image queries are scoped to the 38 live events (`.in("event_id", liveIds)`), which Postgres serves from `idx_images_event_created (event_id, created_at)`: **40 ms** measured, versus ~0.9 s for a global 6 h range scan and a timeout for the exact count. Pixieset-migrated events (`settings.pixiesetCollectionId`, 355 of 393) never make the verdict LIVE. Verdict is an exit code: 0 quiet, 2 LIVE, 1 failed. The event-date check now covers local yesterday..tomorrow, since `toISOString()` gave the UTC date and an evening Pacific event is tomorrow in UTC. Verified live: 3.3 s, exit 0 while the ingest wrote rows 0 m ago; a bad column name exited 1 with the Postgres message; a 120-day window exited 2 naming a real event.
